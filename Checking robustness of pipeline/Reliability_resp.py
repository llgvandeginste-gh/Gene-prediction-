...
for c in clusters:
    for m in methods:
        file_name = f"reliability_cluster_{c}_method_{m}_resp.csv"
...
for c in clusters:
    for m in methods:
        file_path = os.path.join(folder_path, f"reliability_cluster_{c}_method_{m}_resp.csv")
        if os.path.exists(file_path):
...
summary_output_path = os.path.join(folder_path, "pipeline_stability_summary_resp.csv")
